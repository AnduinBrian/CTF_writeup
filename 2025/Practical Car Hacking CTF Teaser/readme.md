...
<br>We have \<UID> \<nR1> \<aR1>, now need another \<nR2> \<aR2>. Based on the log, we can see that a key was pressed 6 times, but we don't know which button was pressed, so I tried the next successful parsed packet, I compiled the crack5 and passed the param in.
```python
